...
Protected Module Secrets
	#tag Method, Flags = &h0
		Sub Load()
		  // Loads secret credentials from an out-of-repo "secrets.env" file so they are
		  // never committed to source control. Safe to call repeatedly (idempotent).
		  //
		  // File location (see secrets.env.example):
		  //   Regular server : /etc/ECHOSCORE/secrets.env
		  //   Xojo Cloud     : SharedDocuments/ECHOSCORE/secrets.env
		  //
		  // The same file works in both places. DB_USERNAME / DB_PASSWORD are used on a
		  // regular server (incl. local debug builds); XOJO_DB_USERNAME / XOJO_DB_PASSWORD
		  // are used on Xojo Cloud. Load() selects the right pair for the build target.
		  Const kAppFolder As String = "ECHOSCORE"
