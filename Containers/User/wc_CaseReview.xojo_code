...
Begin wc_base wc_CaseReview
   ...
   Begin WebCheckbox chkRequiresFullEcho
      ...
      _mPanelIndex    =   -1
   End
   Begin WebTextArea txtConclusions
      AllowReturnKey  =   True
   ...
   Begin WebButton btnSubmit
      ...
      _mPanelIndex    =   -1
   End
   Begin WebLabel lblVideoCounter
      Bold            =   True
      ControlID       =   ""
      CSSClasses      =   ""
      Enabled         =   True
   ...
   Begin WebLabel lblConclusions
      ...
      _mPanelIndex    =   -1
   End
   Begin WebLabel lblCorrectConclusionsTitle
      Bold            =   True
      ControlID       =   ""
      CSSClasses      =   ""
      Enabled         =   True
      ...
      _mPanelIndex    =   -1
   End
   Begin WebLabel lblCorrectConclusions
      Bold            =   False
      ControlID       =   ""
      CSSClasses      =   ""
      Enabled         =   True
   ...
   Begin WebLabel lblScore
      ...
      _mPanelIndex    =   -1
   End
   Begin WebButton btnPreviousCase
      AllowAutoDisable=   False
      Cancel          =   False
      Caption         =   "❮  Previous Case"
      ControlID       =   ""
      CSSClasses      =   ""
      Default         =   False
      Enabled         =   True
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub DisplayCurrentVideo()
		  ...
		  Else
		    ...
		    html = html + "* { margin: 0; padding: 0; box-sizing: border-box; }"
		    html = html + "html, body { height: 100%; width: 100%; overflow: hidden; background: transparent; }"
		    html = html + ".video-wrapper { display: flex; align-items: flex-start; justify-content: flex-start; width: 100%; padding: 10px; }"
		    html = html + ".video-container { width: 100%; max-width: 750px; background: #000; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.15); position: relative; }"
		    html = html + "video { width: 100%; height: auto; display: block; background: #000; cursor: pointer; }"
		    html = html + ".nav-info { position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); color: #fff; font-size: 11px; background: rgba(0,0,0,0.7); padding: 4px 8px; border-radius: 4px; white-space: nowrap; z-index: 10; }"
		    html = html + "</style></head><body>"
	...
	#tag Method, Flags = &h21
		Private Sub LoadCase()
		  ...
		  Catch e As DatabaseException
		    ...
		    lblCaseTitle.Text = "Error Loading Case"
		  End Try
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub NavigateToCase(newCaseID As Integer)
		  ' *******************************************************************************
		  ' NavigateToCase Method
		  ' Navigates to a different case while staying in wc_CaseReview
		  ' *******************************************************************************
		  ' Update the CaseID
		  CaseID = newCaseID
		  ...
		  ' Reload everything for the new case
		  LoadCase
		  LoadExistingResponse
		  LoadVideos
		  UpdateVideoNavigation
	...
	#tag Method, Flags = &h21
		Private Sub ResetUI()
		  ...
		  txtConclusions.ReadOnly = False
		  txtConclusions.Text = ""
		  lblCorrectConclusions.Visible = False
		  lblCorrectConclusionsTitle.Visible = False
	...
	#tag Method, Flags = &h21
		Private Sub SaveResponse(isCompleted As Boolean)
		  ' *******************************************************************************
		  ' SaveResponse Method
	...
	#tag Method, Flags = &h21
		Private Sub ShowCorrectAnswers()
		  ' *******************************************************************************
	...
	#tag Method, Flags = &h21
		Private Sub UpdateCaseNavigation()
		  ...
		  ' Updates the Previous/Next Case buttons
		  ' *******************************************************************************
	...
	#tag Method, Flags = &h21
		Private Sub UpdateVideoNavigation()
		  ' *******************************************************************************
		  ...
		  ' Enable buttons based on video count
		  btnPreviousVideo.Enabled = TotalVideos > 1
		  btnNextVideo.Enabled = TotalVideos > 1
	...
	#tag Property, Flags = &h21
		Private TotalCases As Integer = 0
	#tag EndProperty
